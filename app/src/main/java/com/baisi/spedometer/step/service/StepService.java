...
import com.baisi.spedometer.activity.MainActivity;
...
public class StepService extends Service implements SensorEventListener {
     ...
     * 默认为30秒进行一次存储
     */
    private static int duration = 20 * 1000;
    ...
    /*当天所走的秒数*/
    private static int stepTimeCountSecond = 0;
    ...
    private int stepSecondDruction = 5;

    // 登陆googlefit  多长时间访问googlefit数据库
    private int googlefitSecond = 5 * 1000;
    // sprot  运动模式步骤 ；
    private int SPORTSTEPCOUNT;
    private boolean isSendHeartBeat = false;

    //控制googlefit  登陆成功停止传感计算步逻辑；
    private boolean isGooglefitOnUpdaUi = false;

    /**
     * 保存记步计时器
     */
    private Disposable rxDisposable, rxDisposableTimeCount, rxDisposableGooglefitTimeCount;


    private static int lastHourInt = Calendar.getInstance().get(Calendar.HOUR_OF_DAY);
    private boolean isANewHour = false;
    ...
    public void onCreate() {
        ...
        sharedPreferencesUtils = new SharedPreferencesUtils(PedometerApplication.getApplicationContextPedometer());
    ...
    public void onEventReceive(Event event) {
        switch (event.getCode()) {
            ...
            case EventBusUtil.EventCode.SPORTMODESTOP:
                ...
                SPORTSTEPCOUNT = 0;
                break;
            case EventBusUtil.EventCode.SPORTMODEPAUSETOSTART:
                isSendHeartBeat = true;
                break;
        }
    }
    ...
    private void readFitData() {
        ...
        rxDisposableGooglefitTimeCount = Observable.interval(0, googlefitSecond, TimeUnit.MILLISECONDS)
    ...
    private boolean judGmentGoogleFit() {
        ...
        if (GoogleSignIn.hasPermissions(GoogleSignIn.getLastSignedInAccount(StepService.this), fitnessOptions2)) {
            ...
            Log.d(TAG, "judGmentGoogleFit:  true ");
            return true;
        } else {
            Log.d(TAG, "judGmentGoogleFit:  false ");
            return false;
        }
    ...
    public DataReadRequest queryFitnessData() {
        ...
        Long startTime = StepConversion.getBeginTime();
        Long endTime = StepConversion.getEndTime()  ;
    ...
    private void startStepCountThread() {
        ...
        rxDisposable = Observable.interval(0, stepSecondDruction, TimeUnit.SECONDS).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Consumer<Long>() {
                    @Override
                    public void accept(Long aLong) throws Exception {
                        //while (true) {
                        Log.d(TAG, "run: startStepCountThread ");

                        long a = System.currentTimeMillis();
                        //ArrayList list = new ArrayList(longArrayList);
                        ...
                        if (!longArrayList.isEmpty()) {
                            ...
                            Log.d(TAG, "up date ui  steptimecountsecond :" + stepTimeCountSecond);
                            //save();
                            longArrayList.clear();
                        }
                        //}
    ...
    private void setIsGoogleFitUpdataUi(boolean trueorfalse) {
        isGooglefitOnUpdaUi = trueorfalse;
    }
    ...
    private void initTodayData() {
        ...
        if (list.size() == 0 || list.isEmpty()) {
            ...
            sharedPreferencesUtils.setParam(SharedPreferencesUtils.STEP_NUMBER, 0);
            sharedPreferencesUtils.setParam(SharedPreferencesUtils.STEP_SECOND, 0);
            Log.d(TAG, "initToday");
        } else if (list.size() > 0) {
          ...
          /*  for (int i = 0; i < list.size(); i++) {
            ...
            */
            CURRENT_STEP = (int) sharedPreferencesUtils.getParam(SharedPreferencesUtils.STEP_NUMBER, 0);
            stepTimeCountSecond = (int) sharedPreferencesUtils.getParam(SharedPreferencesUtils.STEP_SECOND, 0);

            if (stepCountAll < 0) {
                CURRENT_STEP = 0;
            }
            if (stepTimeCountSecond < 0) {
                stepTimeCountSecond = 0;
            }
...
//        filter.addAction(Intent.ACTION_USER_PRESENT);
        ...
        mBatInfoReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(final Context context, final Intent intent) {
                ...
                if (Intent.ACTION_SCREEN_ON.equals(action)) {
                    ...
                    //sharedPreferencesUtils.setParam(SharedPreferencesUtils.LAST_HOURSTEP_ALL, CURRENT_STEP);
                    //sharedPreferencesUtils.setParam(SharedPreferencesUtils.LAST_HOURSTEPSECOND_ALL, stepTimeCountSecond);
                    save(false);
                    ...
                    //时间变化步数重置为0
                    Log.d(TAG, "onReceive: ACTION_TIME_CHANGED");
                    isCall();

                } else if (Intent.ACTION_TIME_TICK.equals(action)) {//日期变化步数重置为0   分钟
                    Log.d(TAG, "onReceive: ACTION_TIME_TICK");
                    Calendar calendar = Calendar.getInstance();
                    int thisHourInt = calendar.get(Calendar.HOUR_OF_DAY);
                    isANewHour = false;
                    if (thisHourInt != lastHourInt) {
                        Log.d(TAG, "onReceive:  新的一个小时； ");
                        // 存上一个小时的步骤和运动时间；
                        save(true);
                        isANewHour = true;
                        sharedPreferencesUtils.setParam(SharedPreferencesUtils.LAST_HOURSTEP_ALL, CURRENT_STEP);
                    ...
                    lastHourInt = calendar.get(Calendar.HOUR_OF_DAY);
                }
            }
        };
    ...
    private void updateNotification() {
        sharedPreferencesUtils.setParam(SharedPreferencesUtils.STEP_NUMBER, CURRENT_STEP);
        sharedPreferencesUtils.setParam(SharedPreferencesUtils.STEP_SECOND, stepTimeCountSecond);
        //设置点击跳转
        ...
        if (mCallback != null) {
            Log.d(TAG, "updatauiCallback !=null  and   update step is  " + CURRENT_STEP);
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (isGooglefitOnUpdaUi != true) {
            updateNotification();
            Log.d(TAG ,"sensorChange  " +  " not +++++googlefit" ) ;
            startUpDateStepTimeCountSecond();
        }
    }

    /**
     * 更新 sportActivityUI 的方法
     */
    private void updataSportActUi() {
        if (isSendHeartBeat) {
            Log.d(TAG, "onSensorChanged:   send sportevent tick");
            SPORTSTEPCOUNT++;
            EventBusUtil.sendEvent(new Event(EventBusUtil.EventCode.SPORTMODE_SERVICE_TO_SPORTACTIVITY, SPORTSTEPCOUNT));
        }
    }
    ...
    public void startUpDateStepTimeCountSecond() {
        ...
        Log.d(TAG, "startUpDateStepTimeCountSecond: " + timeOfThisPeak);
     ...
     * 保存记步数据
     */
    private synchronized void save(boolean end) {
        if (googlefitOnUpdaUi) {
            return;
        }
        ...
        if (list.size() == 0) {
            ...
            StepData lastone = daoSession.getStepDataDao().queryBuilder().where(StepDataDao.Properties.Id.eq(countId)).unique();

            int step = (tempStep - (int) sharedPreferencesUtils.getParam(SharedPreferencesUtils.LAST_HOURSTEP_ALL, 0));
            int stepsecond = tempStepSecond - (int) sharedPreferencesUtils.getParam(SharedPreferencesUtils.LAST_HOURSTEPSECOND_ALL, 0);
            if (step < 0) {
                step = 0;
            }
            if (stepsecond < 0) {
            ...
            if (!isANewHour) {
                ...
                if (end) {
                    data.setStepTime(new Date(dateNow.getTime() - 1500));
                } else {
                    data.setStepTime(dateNow);
                }
                lastone.setStepToday(getTodaySimpleDate());
                lastone.setStep(step);
                lastone.setStepSecond(stepsecond);
                ...
                StepData stepData = new StepData();
                stepData.setStep(step); //换下一个小时存储；
                stepData.setStepSecond(stepsecond);
    ...
    public void onDestroy() {
        ...
        if (!rxDisposableGooglefitTimeCount.isDisposed()) {
            if (rxDisposableGooglefitTimeCount != null) {
                rxDisposableGooglefitTimeCount.dispose();
            }
        }
