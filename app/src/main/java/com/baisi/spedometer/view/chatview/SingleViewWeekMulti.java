...
public class SingleViewWeekMulti extends View {
    ...
    private void initStart() {
        ...
        // 点击显示步骤的框的宽度,高度 ,字体大小 ；
        stepTextFrameWidth = densityUtil.dip2px(30);
        stepTextFrameHeight = densityUtil.dip2px(25);
        stepTextSize = densityUtil.dip2px(12);
        ...
        //  柱状图上的步骤显示小三角的宽度；
        trangleWith = densityUtil.dip2px(8);
    }
    ...
    public SingleViewWeekMulti(Context context) {
        this(context, null);
    ...
    public SingleViewWeekMulti(Context context, AttributeSet attrs) {
    ...
    public SingleViewWeekMulti(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (singViewType.equals("today")) {
            ...
            for (int i = 0; i < 7; i++) {
                ...
                if (selectIndexRoles.contains(i)) {
                    ...
                    if (StepConversion.getDayofweek(new Date()) == (i)) {
                        canvas.drawCircle(rectFIndex.centerX(), rectFIndex.centerY(), rectFIndex.width(), mCirclePaint);
                    }
                }
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (list.size() > 0) {
                ...
                if (  list.get(i)!=0f){
                    //画 step  text
                    float top = rectFHistogram.top - (densityUtil.dip2px(10) + stepTextFrameHeight);
                    if (top<=0){
                        top=0;
                    }
                    RectF rectFStepText = new RectF(rectFHistogram.centerX() - stepTextFrameWidth / 2, top, rectFHistogram.centerX() + stepTextFrameWidth / 2, top + stepTextFrameHeight);
                    //if (isPaintStep) {
                    ...
                    mChartPaint.setAntiAlias(true);
                    mChartPaint.setStyle(Paint.Style.FILL);
                    mChartPaint.setColor(getResources().getColor(R.color.report_tv_stroke));
                    canvas.drawRoundRect(rectFStepText, 10, 10, mChartPaint);

                    Path path = new Path();
                    path.moveTo(rectFStepText.centerX() - (float) trangleWith / 2f, rectFStepText.bottom);
                    path.lineTo(rectFStepText.centerX(), rectFStepText.bottom + trangleWith / 2);
                    path.lineTo(rectFStepText.centerX() + (float) trangleWith / 2f, rectFStepText.bottom);
