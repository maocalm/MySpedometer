...
public class MonthReportActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initView();
        initData();
        Firebase.getInstance(getApplicationContext()).logEvent("MonthReportActivity", "进入");

           /*加载广告*/
        GoogleAd.loadAd(adFrameLayout, 0, "MonthReportActivity");
        GoogleAd.LoadFullAd("MonthReportActivity");
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.step_tv:
                performClickReportTv(false, STEP_STR);
                Firebase.getInstance(getApplicationContext()).logEvent("MonthReportActivity", "step_tv 点击");
                break;
            case R.id.calorle_tv:
                performClickReportTv(false, CALORIE_STR);
                Firebase.getInstance(getApplicationContext()).logEvent("MonthReportActivity", "calorle_tv 点击");
                break;
            case R.id.time_tv:
                performClickReportTv(false, TIME_STR);
                Firebase.getInstance(getApplicationContext()).logEvent("MonthReportActivity", "time_tv 点击");
                break;
            case R.id.distance_tv:
                performClickReportTv(false, DISTANCE_STR);
                Firebase.getInstance(getApplicationContext()).logEvent("MonthReportActivity", "distance_tv 点击");
                break;
        }
    }
    ...
    public void performClickReportTv(boolean isFromPedometer, String classification) {
        ...
        switch (classification) {
            ...
            case CALORIE_STR:
                ...
                ArrayList calorlelist = StepConversion.getCalorieList(getStepListOrStepsecondList(STEP_STR));
            ...
            case DISTANCE_STR:
                ...
                ArrayList distancelist = StepConversion.getDistanceList(getStepListOrStepsecondList(STEP_STR));
    ...
    private void setTopTotalAndAverage(String classification) {
        ...
        List calorList = StepConversion.getCalorieList(stepList);
        List distanceList = StepConversion.getDistanceList(stepList);
