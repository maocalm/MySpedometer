...
public class SettingFragment extends BaseFragment implements View.OnClickListener , ShowAdFilter {
    ...
    protected View initViews(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        initRequired();
        initView();
        Firebase.getInstance(getApplicationContext()).logEvent("SettingFragment", "进入");
        return view;
    }
    ...
    * */
    ...
    private void initView() {
        ...
        String gender = mSpUtils.getParam(SharedPreferencesUtils.GENDER ,male).toString();
        ...
        switchCompat.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    ...
                    EventBusUtil.sendEvent(new Event(EventBusUtil.EventCode.STARTNOTIFICATION));
                } else {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.rl_gender:
                ...
                Firebase.getInstance(getApplicationContext()).logEvent("SettingFragment", "性别选择按钮点击");
                break;
            case R.id.rl_height:
                showPopWindow(view_height, true, null);
                Firebase.getInstance(getApplicationContext()).logEvent("SettingFragment", "身高选择按钮点击");

                break;

            case R.id.rl_weight:
                showPopWindow(pop_weight, true, null);
                Firebase.getInstance(getApplicationContext()).logEvent("SettingFragment", "体重选择按钮点击");

                break;
            case R.id.rl_metric:
                showPopWindow(pop_metric, false, tv_show_metric);
                Firebase.getInstance(getApplicationContext()).logEvent("SettingFragment", "单位选择按钮点击");

                break;

            case R.id.rl_setp_goal:
                showPopWindow(pop_stepgoal, true, null);
                Firebase.getInstance(getApplicationContext()).logEvent("SettingFragment", "目标步数选择按钮点击");
            ...
            case R.id.male:
                ...
                popWindow.dissmiss();
                tv_show_gender_male.setBackgroundColor(getResources().getColor(R.color.text_color));
                tv_show_gender_femal.setBackgroundColor(Color.WHITE);
                mSpUtils.setParam(SharedPreferencesUtils.GENDER, getResources().getString(R.string.male));
                Firebase.getInstance(getApplicationContext()).logEvent("设置页", "性别选择", "male");
                break;
            case R.id.femal:
                tv_show_gender.setText(R.string.female);
                popWindow.dissmiss();
                tv_show_gender_femal.setBackgroundColor(getResources().getColor(R.color.text_color));
                tv_show_gender_male.setBackgroundColor(Color.WHITE);
                mSpUtils.setParam(SharedPreferencesUtils.GENDER, getResources().getString(R.string.female));
            ...
            case R.id.stepgoal_ok:
                ...
                break;
            case R.id.swc:
                Firebase.getInstance(getApplicationContext()).logEvent("SettingFragment", "通知选择按钮点击");

                switchCompat.setChecked(!switchCompat.isSelected());
                if (switchCompat.isSelected()) {
                    Firebase.getInstance(getApplicationContext()).logEvent("SettingFragment", "通知选择按钮点击——————开");
                    mSpUtils.setParam(SharedPreferencesUtils.SWITCHBAR, SharedPreferencesUtils.SWITCH_TRUE);
                } else {
                    mSpUtils.setParam(SharedPreferencesUtils.SWITCHBAR, SharedPreferencesUtils.SWITCH_FALSE);
                    Firebase.getInstance(getApplicationContext()).logEvent("SettingFragment", "通知选择按钮点击——————关");
                }
                break;
